...
export async function fetchPartyCourtsForUser(user, gym) {
    ...
    const [
        ...
        { data: queuedPartyCounts, error: queuedPartyCountsError },
    ] = courtIds.length
        ? await Promise.all([
            supabase
                .from("court_parties")
                ...
                .eq("status", "active"),
            supabase.rpc("queued_party_counts_for_courts", {
    ...
    if (queuedPartyCountsError) {
        throw new Error(queuedPartyCountsError.message);
    }

    const activePartiesByCourtId = new Map(
        (activeParties ?? []).map((party) => [party.court_id, party]),
    );
    const queuedPartyCountsByCourtId = new Map(
        (queuedPartyCounts ?? []).map((countRow) => [
            countRow.court_id,
            countRow.queued_party_count,
        ]),
    );
    ...
    return {
        ...
        const activeParty = activePartyRow
            ...
            : null;
        const queuedPartyCount = queuedPartyCountsByCourtId.get(court.id) ?? 0;
...
export async function fetchPartyCourtForUser(user, courtId) {
    const supabase = getSupabaseAdmin();
    ...
    const activePartyRow = activeParties?.[0] ?? null;
    const activeParty = activePartyRow
        ? mapParty(activePartyRow, user, userMembership, userIsAdmin, court.queue_disabled)
        : null;
    const queuedPartyCount = queuedPartyCounts?.[0]?.queued_party_count ?? 0;
    const activeOpenSlots = activeParty?.openSlots ?? 4;
    const userParty = userMembership?.court_parties?.court_id === court.id
        ? {
            id: userMembership.court_parties.id,
...
export async function maybeRotateDuePartyCourts(minIntervalSeconds = 30) {
    const supabase = getSupabaseAdmin();

    const { data: rotatedCount, error } = await supabase.rpc(
        "maybe_rotate_due_party_courts_atomic",
